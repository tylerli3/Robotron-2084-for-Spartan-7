...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if (status != XST_SUCCESS){
		xil_printf("error in writing to reg");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (int n=0; n<nbytes; n++){
		WriteBuffer[n+1] = data[n];
	}
	status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer,nbytes+1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	WriteBuffer[1] = 0x00;
	status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer,2);
	BYTE val = ReadBuffer[1];
	//read return code from SPI peripheral 
	if (status != XST_SUCCESS){
			xil_printf("error in reading reg");
	}
	//if return code != 0 print an error
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0x00000000);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	WriteBuffer[0] = reg;
	for (int n=0; n<nbytes; n++){
				WriteBuffer[n+1] = 0x00;
	}
	status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer,nbytes+1);
	for (int n=0; n<nbytes; n++){
			data[n] = ReadBuffer[n+1];
	}
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (status != XST_SUCCESS){
